...
void Get_input(int my_rank, int comm_sz, double* a_p, double* b_p,
      int* n_p) {
   int divisor, count, core_difference;
   
   count = ceil(log(comm_sz)/log(2));
   ...
   if (my_rank == 0) {
      ...
      fscanf(file, "%lf %lf %d", a_p, b_p, n_p);
      printf("Os valores de entrada sao: a=%lf b=%lf n=%d \n", *a_p, *b_p, *n_p);
	  fclose(file);
   } 
   int i = 0;
   for (i=0; i<count; i++){                                    
     if (my_rank%divisor==0 && (my_rank+core_difference < comm_sz)) 
     {
